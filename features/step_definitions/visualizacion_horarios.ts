...
Given(
  'que se tiene un archivo tipo JSON con el horario generado previamente llamado visualizacion_horario_prueba',
  { timeout: 5 * 5000 },
  async function () {
    /*
...
When(
  'se seleccione un filtro por el docente Martínez Mosquera Silvia Diana',
  { timeout: 5 * 5000 },
  async function () {
    /*
    Los datos recibidos son:
    ...
    */
    ...
    this.resultadoObtenido = await this.horarioController.obtenerHorarioDocente(
      this.nombreDocente,
      this.horarioGenerado.id,
    );
  },
);

Then(
  'se obtendrá las asignaturas, semestres y horas del docente seleccionado respuesta_horario_docente.',
  { timeout: 5 * 5000 },
  async function () {
    // Leer archivo
...
When(
  'se seleccione un filtro por semestre de una carrera y grupo 6-GR1-ICC',
  { timeout: 5 * 5000 },
  async function () {
    /*
    Los datos recibidos son:
    ...
    */
    ...
    // Llamar al método de filtrado
    this.resultadoObtenidoFiltro =
      await this.horarioController.obtenerHorarioGrupo(
        this.filtroSemestreCarreraGrupo,
        this.horarioGenerado.id,
      );
  },
);
...
Then(
  'se obtendrá las asignaturas, docente y horas del filtro seleccionado respuesta_horario_carrera.',
  { timeout: 5 * 5000 },
  async function () {
    /*
    Los datos recibidos son:
    ...
    */
    ...
    const pathArchivoFiltro = path.join(
      __dirname,
      '..',
      'documents_test',
      this.nombreArchivoLeerFiltro,
    );
    this.lecturaArchivoRespuestaFiltro = fs.readFileSync(pathArchivoFiltro);
    assert.deepEqual(
      this.resultadoObtenidoFiltro,
      JSON.parse(this.lecturaArchivoRespuestaFiltro),
    );
  },
);
